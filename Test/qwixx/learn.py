...
class FeatureHasher:
    ...
    def hash_state(self, g: QwixxGame, player_name: str) -> str:
        ...
        # binned white-sum (reduce fragmentation)
        d = g.dice
        ...
        if ws == 0:
            ...
            ws_bin = 'H'
        parts.append(f"WS:{ws_bin}")

        # frontiers + counts + closed
        for c in COLORS:
...
def plan_key(white_action: Tuple[str, Optional[str]], color_action: Tuple[str, Optional[str], Optional[int]]) -> str:
    ...
    return f"W:{w_act}:{w_col or '-'}\nC:{c_act}:{c_col or '-'}:{c_wd if c_wd else '-'}"
