...
from .ai import HeuristicBot, MCTSBot, LearnerBot, legal_color_actions
...
def _apply_white_for_all(g: QwixxGame, bots: Dict[str, object]):
    ...
    for p in g.players:
        ...
        if p.name == active and getattr(bot, 'pending_plan', None):
            act, color = bot.pending_plan.get('white_action', ('skip', None))
        else:
            act, color = bot.choose_white_sum_action(g)
        try:
